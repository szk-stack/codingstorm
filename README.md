...
| `queued` | 排队中 | 取消 |
| `running` | 正在执行 | 等 |
| `awaiting_review` | 跑完了，等你看 diff | 批准 / 丢弃 |
| `merged` | 已合入主干（终态） | — |
| `discarded` | 已丢弃（终态） | — |
| `cancelled` | 排队时被取消（终态） | — |
| `failed` | 执行失败 | 重新入队 / 丢弃 |
...
`requeue` 会**重建工作区** —— 旧工作区带着上次未提交的改动、且基于旧基点，留着只会误导。
...
- 任务详情两个页签：**执行过程**（实时推送，工具调用渲染成可展开卡片）和**改动**（diff + 批准/丢弃按钮）。
- 路由是 hash 形式的 `#/task/<id>` 和 `#/task/<id>/diff`，链接可分享、刷新回到原处。
